...
export default function VideoPortfolio(): React.ReactElement {
  const [isMuted, setIsMuted] = useState(false);

  return (
    <div className="video-portfolio-section">
      <div className="video-portfolio-header">
        <h2 className="video-portfolio-title">
          {'VIDEO PORTFOLIO'.split('').map((letter, index) => (
            <span
              key={index}
              className="video-title-letter"
            >
              {letter === ' ' ? '\u00A0' : letter}
            </span>
          ))}
        ...
        {videos.map((video, index) => (
          <div key={index} className="video-card">
            ...
            <div className="video-overlay">
              <span className="video-text">{video.title}</span>
            </div>
          </div>
        ))}
      </div>
